...
{
   	echo "<script>alert('不允许空值');location.href='index.php'</script>";
}
else
{
   if( "$_GET[china]"<0 ||  "$_GET[maths]"<0)
   {
   	echo "<script>alert('分数不能为负');location.href='index.php'</script>";
   }
   else
   {
      if (mysql_num_rows($result))
       ...
       if (!mysql_query($sql,$con))
       {
          ...
          echo "<script>alert('学生成绩记录修改成功');location.href='index.php'</script>";
       }
   }
   else{	
	     echo "<script>alert('没有此记录，成绩修改失败');location.href='index.php'</script>";
  }
}}
mysql_close($con)
?>